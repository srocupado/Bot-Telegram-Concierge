...
async def consultar_sessoes(
    ...
    """Horários das sessões de um filme num cinema, numa data (default hoje).
    Texto pronto pra repassar no chat."""
    ...
    if not cands:
        return (f"Não achei o cinema “{cinema}” no diretório Cinemark. "
    ...
    async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
        ...
        fq = {t for t in _norm(filme).split() if t}
        melhor, melhor_score = None, 0
        for m in em_cartaz:
            mt = set(_norm(m.get("name") or "").split())
            sc = len(fq & mt)
            if sc > melhor_score:
                melhor, melhor_score = m, sc
        if not melhor:
            lista = ", ".join(m.get("name", "?") for m in em_cartaz[:10])
            return (f"“{filme}” não está em cartaz no {th['name']}. "
                    f"Em cartaz: {lista}.")
    ...
    nome_filme = res.get("movieName") or melhor.get("name") or filme
