...
_SUMMARY_SYSTEM = (
    ...
    "texto.\n"
    "PROIBIDO registrar ESTADO DO SISTEMA do bot: monitoramentos/watches "
    "criados ou ativos, lembretes agendados, tarefas abertas, lançamentos "
    "feitos. Esses vivem no banco de dados e são consultados por ferramenta "
    "na hora — gravar aqui vira mentira quando o item é cancelado (caso "
    "real: 'criou monitoramento do drone' ficou no resumo, o watch foi "
    "removido, e o bot passou a afirmar pra sempre que o monitoramento "
    "estava ativo). Se o resumo anterior contiver itens assim, REMOVA-os "
    "na atualização.\n"
    "Formato: bullets curtos '- ...', agrupados por tema quando "
...
async def _compact_serializado(user_id: int, msgs: list[ChatMessage]) -> None:
    ...
    O `old` é relido AQUI, já dentro do lock — é isso que garante que a segunda
    compactação incorpora o resultado da primeira em vez de sobrescrever."""
    async with _sessionmaker() as session:
