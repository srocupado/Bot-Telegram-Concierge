...
async def _run_congresso(bot: Bot, chat_id: int) -> None:
    ...
    # noite a _week_bounds pulava pra semana SEGUINTE da pauta.
...
async def _run_chat(
    ...
    try:
        ...
        # Mesmo contrato do chat ao vivo: tz efetivo (viagem) e user_text —
        # sem o user_text, as travas determinísticas (banco→cartão, parcelas)
    ...
    # Honra o contrato do ToolContext, como o deliver_llm_reply do chat ao
    # vivo: tool verbatim (saldo, lembretes, congresso, consultar_mp_dou…)
    # seta direct_html + short_circuit e o provider devolve "" — sem isto o
    # usuário recebia "⏰ (agendado)" VAZIO e o conteúdo (inclusive o aviso
    # "não consegui checar o DOU") era descartado em silêncio.
    if ctx.direct_html:
    ...
    if not (reply or "").strip():
        # Turno assistant vazio na memória envenena as chamadas seguintes
        # (a API da Anthropic rejeita content vazio) — grava o placeholder.
        reply = "(sem resposta)"
    memory.append(chat_id, "user", prompt)
