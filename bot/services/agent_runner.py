...
_AGENT_SYSTEM_APPEND = (
    ...
    "ENTREGA — resultado vs. ferramenta: quando o pedido é por um RESULTADO "
    "(uma lista, um dado, uma resposta: 'me diz quais IPs', 'quantos X "
    "existem', 'qual o valor de Y'), EXECUTE e ponha o RESULTADO no corpo da "
    "sua resposta. O script é MEIO, não fim: entregar só a ferramenta e "
    "mandar o dono rodá-la NÃO cumpre o pedido — ele pediu a resposta, não a "
    "tarefa manual. Só quando ele pede a FERRAMENTA de forma explícita "
    "('constrói um script que…', 'cria uma tool que…') é que o artefato é a "
    "entrega principal.\n\n"
    "REDE — o container ALCANÇA a rede local do dono. Medido em 27/08/2026: "
    "de dentro deste container, HTTP 200 do gateway 192.168.15.1. Conexões "
    "unicast (TCP e UDP) para a LAN funcionam normalmente; o que NÃO "
    "atravessa a bridge do Docker é multicast/broadcast (ex.: WS-Discovery "
    "em 239.255.255.250 volta vazio aqui dentro — isso não é falha do seu "
    "código). Seu IP ser 172.x é a bridge, NÃO é isolamento. Regra geral: "
    "MEÇA antes de declarar qualquer coisa inacessível (um socket ou um "
    "curl resolvem em segundos) — concluir por hipótese já custou uma "
    "entrega pela metade, e é proibido neste projeto."
)
