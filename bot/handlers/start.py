...
HELP_TEXT = (
    ...
    "• <code>/mp_dou_agora [AAAA-MM-DD]</code> — busca agora; entrega nota técnica (gerada por IA) + DOCX. A nota roda em SEGUNDO PLANO (leva alguns minutos): o bot confirma na hora e continua respondendo você normalmente enquanto gera. Checagem manual conclusiva (dia já fechado, fonte completa) também dá baixa na fila de re-checagem — o bot avisa quando isso acontece\n"
    "• <code>/mp_em_fila</code> (ou <code>/mp_fila</code>) — mostra o que está na fila: notas técnicas aguardando geração (quando o Inlabs voltar) e dias ainda não verificados que serão re-checados. Só consulta, não altera nada\n"
    ...
    "filmes em cartaz no seu Cinemark (padrão: Iguatemi Brasília). "
    "<code>/fds_cinema &lt;nome&gt;</code> troca o cinema pelo próprio bot "
    "(ex.: <code>/fds_cinema Pier 21</code>), <code>/fds_cinema off</code> "
    "tira a parte de cinema, <code>/fds_cinema padrao</code> volta ao padrão. "
    "Em viagem, clima e lembretes seguem o destino e o cinema fica de fora "
    "sozinho. O <code>/proativo_agora</code> mostra o bloco do próximo fds em "
    "qualquer dia, pra testar\n"
    ...
    "acha o preço e onde comprar (não precisa de comando). O bot ainda <b>abre "
    "a página da melhor oferta pra confirmar o preço na fonte</b> — se a loja "
    "bloquear a leitura, ele avisa que o valor é só referência.\n"
    ...
    "<code>/transito_agora casa</code>. Vale pra <b>qualquer</b> comando desta "
    "ajuda — inclusive <code>/viagem</code>, <code>/mp_dou_agora</code>, "
    "<code>/proativo</code>, <code>/reset_memoria</code> e "
    "<code>/apagar_lembrete</code>.\n"
